...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (name === "soc2-version") options.soc2Version = next();
    else throw new Error(`Unknown option "${value}".`);
  }
  return { target: target ?? "soc2-program", options };
}

function printHelp() {
  console.log(`create-soc2 [directory] [options]

Create a plain-file SOC 2 GRC repository.

Options:
  --company-name <name>
  --policy-owner-name <name>
  --security-contact-email <email>
  --soc2-version <version>       Override resolved engine version
