...
function renderPolicyApprovalGuidance() {
  ...
  const proposalRows = proposals.map((proposal) => '<article><strong>' + esc(proposal.title) + '</strong><p>' + esc(proposal.message) + '</p><div class="evidence-map-references">' + proposal.policyIds.map((id) => formatReference(id)).join("") + '</div></article>').join("");
