...
`docs/data-model.md` is generated from the active model v5. `npm run validate` fails when the generated document differs from the registry. The migration command previews and applies each model upgrade as a separate atomic step, including the explicit v3-to-v4 System classification and relationship rewrite and the v4-to-v5 governed Document lifecycle split.
...
1. Define scope.
2. Approve Policy requirements and complete and independently approve the intended values and exact revisions of required program plans and schedules.
3. Implement Controls and the approved Document requirements, complete authoritative evidence sources, enable schedules, activate unchanged approved Documents with separate dates and revision bindings, and activate Policies at the cutover.
...
The Evidence Ready gate requires an assurance goal, selected Systems, reviewed criteria and service commitments, active and effective required Policies, separately approved and activated required program Documents, active training needed by the selected Controls, implemented Controls, complete authoritative evidence sources, and enabled schedules. Step 2 checks the Document owner, independent approver, approval date, intended values, and approved content revision. Step 3 separately checks linked Control implementation, the active Person who performed activation, Document activation date, effective date, and activation revision. `workflowScope` assigns each Document to the program path or one engagement; Document kind names no longer decide that lifecycle in model v5. Every selected Control family must point to active authoritative Components with the required evidence roles, current evidence access owners, and repeatable retrieval instructions in Record Markdown. These checks are part of Control implementation in `assessProgramReadiness`. The Step 3 browser overview shows Document and Policy activation assessments beside the source-family results, and the CLI and HTTP API expose the same data. Step 4 begins operation and evidence collection only after the program gate. Engagement-scoped Documents remain in Step 5 and are checked by Audit Readiness instead of the program Document gate.
...
For enabled Obligations whose governing Policies and required program Documents are active and effective and that have an implemented linked Control, the dashboard derives the next calendar occurrence from the recurrence rule and the latest applicable Policy or Document effective date. Explicit operational due dates take precedence. Each completed occurrence remains a separate operating record with its own evidence. Starter Obligations can be enabled during implementation but remain dormant until activation.
...
The guided workflow first shipped in model v3. Model v4 separated Workspace identity, Program scope, bounded Systems, operational Components, Vendors, Assets, and Evidence Artifacts. Model v5 is current and separates governed Document approval from activation, assigns Documents to program or engagement workflow scope, and records the Person who performs activation. A v4 active Document that still needs a distinct cutover migrates to approved with its known approval facts and proposed prior effective date intact; the migration never invents an activation actor, date, or revision. Management implements or confirms the linked requirements and records the actual Step 3 activation afterward. Historical management Documents linked to an issued, delivered, or completed Audit remain active with `activationBasis: legacy-v4`, so migration preserves the record and exposes that model v4 had no separate activation event.
