...
- Git exclusively owns version-control facts: authors, commit timestamps, messages, diffs, revisions, renames, and prior file versions. FileGRC records business dates, approvals, effective decisions, and exact Git bindings only when a domain or audit fact must identify reviewed source content.
- Each mutable program fact has one authoritative resource. Policies and governed documents state durable rules and outcomes, then reference the current people, vendors, systems, reporting channels, schedules, targets, inventories, and other facts instead of copying their values. A Reporting Channel Set holds the normal and fallback ways people send a report for one purpose.
- An Obligation occurrence stays rolled up when one queue-level owner, window, population rule, and reconciliation conclusion govern the work. Its reconciliation holds member operating records, evidence, completion, exception, and non-applicability facts. Separate Obligations or Action Items exist only when a member needs its own queue owner, deadline, conclusion, or follow-up lifecycle.
- Headline progress counts independently completable human workflow events, not records or derived checks. When a collection has a distinct record-proposal phase, preparing and reviewing a nonempty collection as a batch contributes one preparation unit and one review unit regardless of its size; collections without that phase and empty collections contribute only their review. Data Retention Schedule rows are an explicit exception because the owner completes each row decision before one whole-schedule approval. Derived coverage and relationship checks explain or block work without adding progress units.
...
`docs/data-model.md` is generated from the active model v11. `npm run validate` fails when the generated document differs from the registry. The migration command previews and applies each model upgrade as a separate atomic step, including the explicit v3-to-v4 System classification and relationship rewrite, the v4-to-v5 governed Document lifecycle split, the v5-to-v6 Training lifecycle and scheduling split, the v6-to-v7 cleanup of model-version lifecycle labels, the v7-to-v8 structured retention, mapping, and supplemental-source review work, the v8-to-v9 rolled-up Obligation and temporal Collection Review workflow, the v9-to-v10 Reporting Route Set workflow, and the v10-to-v11 Control oversight change.
...
It defaults the program lead email from the security contact, defaults the timezone from the local environment, and generates the initial effective date from the current date. Create-time values populate their authoritative Workspace, Person, Appointment, and Reporting Channel Set records. Governed Markdown refers to those authorities instead of copying their current values. Creation fails if the template contains an undeclared token or if any token remains after rendering.
...
1. Define scope.
2. Review Policies, program Documents, and Training on the Policies page; define the Data Retention Schedule document and rows on its page; and independently approve each governed revision.
3. Implement Controls and approved governed-content requirements, complete authoritative evidence sources, configure Obligations, verify those sources against the approved Data Retention Schedule, activate unchanged approved Documents and Training with separate dates and revision bindings, and activate Policies at the cutover.
...
- Approve Policies: independent approval of Policies, program Documents, Training, and the Data Retention Schedule document and structured rows, without requiring implementation or activation
- Implement Controls: the starter control set, authoritative evidence source mappings and readiness, retention implementation coverage, implementation fields, enabled Obligations, complementary customer or subservice controls, operation-tracking status, and the Policy activation cutover
...
The Controls page explains what the generated workspace already supplies and what management must tailor before a planned Control becomes implemented. Its operation-tracking column distinguishes Controls linked to recurring or event work in FileGRC from Controls supported by Evidence Artifacts, and it always shows authoritative evidence-source mappings. A Control owner records implementation without a separate per-Control approval, so that status change raises readiness immediately. The Step 2 overview links to separate Policies and Data Retention Schedule pages. The Policies page combines Policies, program Documents, and Training for individual content approval. The Data Retention Schedule page presents the governed schedule document, derived coverage gaps, structured rows, and one Git-bound schedule review whose collection revision includes the document Markdown and rows. Step 3 links, in order, to Controls, Complementary Controls, Evidence Sources, and Obligations. Evidence Sources is a derived coverage page whose fixes remain in canonical Control and Component records, and Step 3 retains derived checks that implemented sources follow the approved schedule. The Finish Step 3 row shows the next blocker, reveals one Git-bound Control collection review when implementation is ready, then reveals Program Content Activation as management's final cutover. Browser, HTTP API, and CLI use the same domain operations and readiness calculations.
...
Recurring obligations that apply per person, Vendor, System, or other scope item stay as one rolled-up Work Item when the owner, window, population rule, and reconciliation conclusion are shared. The activity profile defines the eligibility rule and evaluation date. Retain the scope revision or population evidence needed to reconstruct the expected members, then reconcile each member to its operating record, approved Exception, or reviewed non-applicability decision. Split the work only when a member needs a separate owner, deadline, conclusion, or follow-up lifecycle.
